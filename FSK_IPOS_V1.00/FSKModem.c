...
int ModulateBit(int type,short* retData)
{
	...
	case 1: // 如果当前比特是1，则添加1 1的波形是有变化
		for(i = 0; i < MODULATE_SAMPLE/2; i++)
		{
			*(retData + index++) = ((g_bIobitFlag == 1) ?AMP_U:AMP_D);
		}
		g_bIobitFlag = !g_bIobitFlag;
		for(i = 0; i < MODULATE_SAMPLE/2; i++)
		{
			*(retData + index++) = ((g_bIobitFlag == 1) ?AMP_U:AMP_D);
		}
...
对data所作的偏移
*/
int PackCount(BYTE* data)
{
	int offset = 0;
	g_cPackageCount ++;
	g_cPackageCount = g_cPackageCount % 256;
	//BYTE* count = (BYTE*)&m_cPackageCount;
	data[offset++] = g_cPackageCount;
	return offset;
...
对data所作的偏移
*/
int PackLength(int len,BYTE* data)
{
	int offset = 0;
	data[offset++] = len % 256;
	data[offset++] = len / 256;
...
对data所作的偏移
*/
int PackCRC(short crc,BYTE* data)
{
	int offset = 0;
	data[offset++] = crc % 256;
	data[offset++] = crc / 256;
...
crc校验结果
*/
short CalculateCRC(BYTE *buf,int len)
{
	BYTE hi,lo;
	int i;
	BYTE j;
...
short* Modulate(char* data,int len,int* outFrameLen,BYTE MobileType)
{
	...
	int voiceLen;
	// 开始调制
	packBuf = PackField(data,len,&packageLen);
	if(packBuf == NULL)
	{
		*outFrameLen = 0;
		return NULL;
	}
	g_nSampleLevel = MobileType; // 调整频率
	
	voiceLen = packageLen * MODULATE_SAMPLE * BYTE_LEN;
	//voiceData = new short[voiceLen];
	voiceData = (short*)malloc(voiceLen * sizeof(short));
...
void FindFrame(short *pData, unsigned long len, long *start, long *end)
{
	unsigned long i, j;
	unsigned long E;
	BYTE flag = 0;
...
int GetValidData(short *InDataBuf,short *OutDataBuf,unsigned long lenth)
{
...
使用的资源 
******************************************/
BYTE FindHead(short * InDataBuf,unsigned long lenth,unsigned long *endlen,BYTE MobileType)
{
	unsigned long i = 0;//指向第i个点
...
使用的资源 
******************************************/
void DisInterference(short *InDataBuf,unsigned long lenth,BYTE MobileType)
{
	unsigned long i = 0;//指向第i个点
